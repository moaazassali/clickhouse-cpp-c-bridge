...
extern "C" EXPORT inline ClickHouseResultStatus CreateColumnArray(Column *inColumn, ColumnArray **outColumn) {
    return TryCatchClickHouseError([&]() {
        *outColumn = new ColumnArrayT(make_fake_shared(inColumn));
    });
}

extern "C" EXPORT inline int32_t ColumnArrayItemType(const ColumnArray *column) {
...
extern "C" EXPORT inline void ColumnArrayAddOffset(ColumnArray *column, const size_t offset) {
    static_cast<CustomColumnArray *>(column)->AddOffsetCustom(offset);
}

extern "C" EXPORT inline size_t ColumnArrayGetOffset(ColumnArray *column, const size_t n) {
